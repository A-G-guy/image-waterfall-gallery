...
- ✨ 在插件设置中添加动态版本号显示
- 🐛 修复移动端画廊标签识别问题，使用增强重试机制
- 🔧 优化事件处理：桌面端使用 `switch-protyle`，移动端使用 `loaded-protyle-static`
- ⚡ 增强重试机制：移动端 6 次重试，总延迟 4200ms
- 🎯 添加手动检测画廊按钮，使用最强保障机制（10 次重试，总延迟 13000ms）
- 📱 为移动端添加 500ms 初始延迟，确保数据库同步
